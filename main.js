...
var petroBlankCloseArray = new Array();
var petroBlankOpenArray = new Array();
var animatePetroBlankTimer;
var movePetroBlankTimer;
var openAndClosePetroBlankCounter = 0;

var hmhcoDiv = document.getElementById("hmhco");
var hmhcoBlankCloseArray = new Array();
var hmhcoBlankOpenArray = new Array();
var animateHmhcoBlankTimer;
var moveHmhcoBlankTimer;
var openAndCloseHmhcoBlankCounter = 0;
...
var piechartAscendiaFrontDiv = document.getElementById("piechart-petro-front");
var piechartAscendiaTextGraphic1Div = document.getElementById("piechart-petro-text-graphic-1");
var piechartAscendiaTextGraphic2Div = document.getElementById("piechart-petro-text-graphic-2");
var piechartAscendiaTextAnimation1Div = document.getElementById("piechart-petro-text-animation-1");
var piechartAscendiaTextAnimation2Div = document.getElementById("piechart-petro-text-animation-2");
var piechartAscendiaTextCode1Div = document.getElementById("piechart-petro-text-code-1");
var piechartAscendiaTextCode2Div = document.getElementById("piechart-petro-text-code-2");

var piechartAscendiaFrontDiv = document.getElementById("piechart-hmhco-front");
var piechartAscendiaTextGraphic1Div = document.getElementById("piechart-hmhco-text-graphic-1");
var piechartAscendiaTextGraphic2Div = document.getElementById("piechart-hmhco-text-graphic-2");
var piechartAscendiaTextAnimation1Div = document.getElementById("piechart-hmhco-text-animation-1");
var piechartAscendiaTextAnimation2Div = document.getElementById("piechart-hmhco-text-animation-2");
var piechartAscendiaTextCode1Div = document.getElementById("piechart-hmhco-text-code-1");
var piechartAscendiaTextCode2Div = document.getElementById("piechart-hmhco-text-code-2");
...
{
	kgmDiv.style.left = experience4ContainerDiv.offsetWidth + "px";
...
{
	kgmDiv.style.left = experience5ContainerDiv.offsetWidth + "px";
			...
			((pageVerticalPosition + (0.5 * containerDiv.offsetWidth) > landInformationContainerArray[i].offsetLeft) && (pageVerticalPosition + (0.5 * containerDiv.offsetWidth) < landInformationContainerArray[i].offsetLeft + landInformationContainerArray[i].offsetWidth)))
				{
					...
					{
						if (canAnimatePetroInformation == false) //if true, animateKgmBlank will be execute after kgm finish move from left to right
						{
							animatePetroBlank();
						}
						else
					...
					{
						if (canAnimateHmhcoInformation == false) //if true, animateKgmBlank will be execute after kgm finish move from left to right
						{
							animateHmhcoBlank();
						}
						else
...
function animatePiechartKgmFront()
{
	...
	{
		$(piechartKgmFrontDiv).stop().animate({opacity: 1}, 500, function() {animatePiechartKgmText()}); //animate piechart
	}
}

function animatePetro()
{
	$(petroDiv).stop().animate({left: "380px"}, 1000, function() {animatePiechartPetroFront(); animatePiechartPetroText(); animatePetroBlank()});
}

function animatePetroBlank()
{
	//animate petro hand right away for one time only
	spinPetroBlank();
	
	//animate petro hand continuously
	clearInterval(animatePetroBlankTimer);
	animatePetroBlankTimer = setInterval(function(){spinPetroBlank()}, 4000);
}

function spinPetroBlank()
{
	clearInterval(spinPetroBlankTimer);
	spinPetroBlankTimer = setInterval(function(){changePetroBlank()}, 100);
}

function changePetroBlank()
{
	if (changePetroBlankCounter >= petroBlankChildrenLength)
	{
		changePetroBlankCounter = 0;
		clearInterval(spinPetroBlankTimer);
		setPetroBlankToDefault();
		
		if ((pageVerticalPosition + (0.5 * containerDiv.offsetWidth) < experience4ContainerDiv.offsetLeft) || (pageVerticalPosition + (0.5 * containerDiv.offsetWidth) > experience1ContainerDiv.offsetLeft + experience4ContainerDiv.offsetWidth))
		{
			clearInterval(animatePetroBlankTimer);
		}
	}
	...
	{
		for (var i=0; i<petroBlankChildrenLength; i++)
		{
			if (i == changePetroBlankCounter)
			{
				setPetroBlankToOpaque(i);
			}
			else
			{
				setPetroBlankToTransparent(i);
			}
		}
	}
	
	changePetroBlankCounter = changePetroBlankCounter + 1;
}

function setPetroBlankToDefault()
{
	for (var i=0; i<petroBlankChildrenLength; i++)
	{
		if (i == 0)
...
function setPetroBlankToOpaque(petroBlankNumber)
{
	//use javascript instead jquery to avoid opacity glitch since this trigger by a very quick timer
				
	petroBlankLeftDiv.children[petroBlankNumber].style.opacity = 1;
	petroBlankLeftDiv.children[petroBlankNumber].style.filter = "alpha(opacity=" + 100 + ")";
				
	petroBlankRightDiv.children[petroBlankNumber].style.opacity = 1;
	petroBlankRightDiv.children[petroBlankNumber].style.filter = "alpha(opacity=" + 100 + ")";
}

function setPetroBlankToTransparent(petroBlankNumber)
{
	//use javascript instead jquery to avoid opacity glitch since this trigger by a very quick timer
				
	petroBlankLeftDiv.children[petroBlankNumber].style.opacity = 0;
	petroBlankLeftDiv.children[petroBlankNumber].style.filter = "alpha(opacity=" + 0 + ")";
				
	petroBlankRightDiv.children[petroBlankNumber].style.opacity = 0;
	petroBlankRightDiv.children[petroBlankNumber].style.filter = "alpha(opacity=" + 0 + ")";
}

function animateHmhco()
{
	$(hmhcoDiv).stop().animate({left: "380px"}, 1000, function() {animatePiechartHmhcoFront(); animatePiechartHmhcoText(); animateHmhcoBlank()});
}

function animateHmhcoBlank()
{
	//animate hmhco hand right away for one time only
	spinHmhcoBlank();
	
	//animate petro hand continuously
	clearInterval(animateHmhcoBlankTimer);
	animateHmhcoBlankTimer = setInterval(function(){spinHmhcoBlank()}, 4000);
}

function spinHmhcoBlank()
{
	clearInterval(spinHmhcoBlankTimer);
	spinHmhcoBlankTimer = setInterval(function(){changeHmhcoBlank()}, 100);
}

function changeHmhcoBlank()
{
	if (changePetroBlankCounter >= hmhcoBlankChildrenLength)
	{
		changeHmhcoBlankCounter = 0;
		clearInterval(spinHmhcoBlankTimer);
		setHmhcoBlankToDefault();
		
		if ((pageVerticalPosition + (0.5 * containerDiv.offsetWidth) < experience5ContainerDiv.offsetLeft) || (pageVerticalPosition + (0.5 * containerDiv.offsetWidth) > experience1ContainerDiv.offsetLeft + experience5ContainerDiv.offsetWidth))
		{
			clearInterval(animateHmhcoBlankTimer);
		}
	}
	...
	{
		for (var i=0; i<hmhcoBlankChildrenLength; i++)
		{
			if (i == changeHmhcoBlankCounter)
			{
				setHmhcoBlankToOpaque(i);
			}
			else
			{
				setHmhcoBlankToTransparent(i);
			}
		}
	}
	
	changeHmhcoBlankCounter = changeHmhcoBlankCounter + 1;
}

function setHmhcoBlankToDefault()
{
	for (var i=0; i<hmhcoBlankChildrenLength; i++)
	{
		if (i == 0)
...
function setHmhcoBlankToOpaque(hmhcoBlankNumber)
{
	//use javascript instead jquery to avoid opacity glitch since this trigger by a very quick timer
				
	hmhcoBlankLeftDiv.children[hmhcoBlankNumber].style.opacity = 1;
	hmhcoBlankLeftDiv.children[hmhcoBlankNumber].style.filter = "alpha(opacity=" + 100 + ")";
				
	hmhcoBlankRightDiv.children[hmhcoBlankNumber].style.opacity = 1;
	hmhcoBlankRightDiv.children[hmhcoBlankNumber].style.filter = "alpha(opacity=" + 100 + ")";
}

function setHmhcoBlankToTransparent(petroBlankNumber)
{
	//use javascript instead jquery to avoid opacity glitch since this trigger by a very quick timer
				
	hmhcoBlankLeftDiv.children[hmhcoBlankNumber].style.opacity = 0;
	hmhcoBlankLeftDiv.children[hmhcoBlankNumber].style.filter = "alpha(opacity=" + 0 + ")";
				
	hmhcoBlankRightDiv.children[hmhcoBlankNumber].style.opacity = 0;
	hmhcoBlankRightDiv.children[hmhcoBlankNumber].style.filter = "alpha(opacity=" + 0 + ")";
}
...
{
	$(piechartPetroTextCode1Div).stop().delay(100).animate({opacity: 1}, 1000, function() {});
	$(piechartPetroTextCode2Div).stop().delay(100).animate({opacity: 1}, 1000, function() {});
}

function animatePiechartPetroTextGraphic()
{
	$(piechartPetroTextGraphic1Div).stop().delay(400).animate({opacity: 1}, 1000, function() {});
	$(piechartPetroTextGraphic2Div).stop().delay(400).animate({opacity: 1}, 1000, function() {});
}

function animatePiechartPetroTextAnimation()
{
	$(piechartPetroTextAnimation1Div).stop().delay(700).animate({opacity: 1}, 1000, function() {});
	$(piechartPetroTextAnimation2Div).stop().delay(700).animate({opacity: 1}, 1000, function() {});
}
...
function animatePiechartPetroTextCode()
{
	$(piechartHmhcoTextCode1Div).stop().delay(100).animate({opacity: 1}, 1000, function() {});
	$(piechartHmhcoTextCode2Div).stop().delay(100).animate({opacity: 1}, 1000, function() {});
}

function animatePiechartHmhcoTextGraphic()
{
	$(piechartHmhcoTextGraphic1Div).stop().delay(400).animate({opacity: 1}, 1000, function() {});
	$(piechartHmhcoTextGraphic2Div).stop().delay(400).animate({opacity: 1}, 1000, function() {});
}

function animatePiechartHmhcoTextAnimation()
{
	$(piechartHmhcoTextAnimation1Div).stop().delay(700).animate({opacity: 1}, 1000, function() {});
	$(piechartHmhcoTextAnimation2Div).stop().delay(700).animate({opacity: 1}, 1000, function() {});
}
